...
public class MainActivity extends AppCompatActivity {
    ...
    private void login() {
        String url = Uri.parse(Config.URL + "login.php").buildUpon()
                .appendQueryParameter("user",etUser.getText().toString())
                .appendQueryParameter("pass",etPass.getText().toString()).build().toString();
    ...
    private void respuesta(JSONObject response){
        try {
            ...
            if(response.getString("login").compareTo("y")==0){
                ...
                Log.d("fh","Error de usuario y/o contraseña ");
            }else{
